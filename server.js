...
const CACHE_TTL = parseInt(process.env.CACHE_TTL) || 300000; // 5min
...
// Export con reward por categoría
...
app.get("/api/client-metrics/:storeId", (req, res) => {
  ...
  const storeId = safe(req.params.storeId);
  if (!storeId) return res.status(400).json({ error: "storeId required" });
  const fromTs = parseInt(req.query.from) || null;
  const toTs = parseInt(req.query.to) || null;
  let whereClause = "";
  let params = [storeId];
  if (fromTs && toTs) {
    whereClause = "AND s.started_at BETWEEN ? AND ?";
  ...
  const variants = db.prepare(`
    ...
    WHERE s.store_id = ? ${whereClause} GROUP BY s.variant
  `).all(...params);
  ...
  res.json({
    storeId, period: { from: fromTs || 0, to: toTs || 0 },
    kpis: {
...
// Endpoint stock (simple, con autenticación para escritura)
...
app.get("/client/:storeId/dashboard", (req, res) => {
  ...
  res.send(`
    ...
    <div class="container">
        ...
        <div class="controls">
            <select id="preset"><option value="7">Últimos 7 días</option><option value="30" selected>Últimos 30 días</option><option value="90">Últimos 90 días</option><option value="0">Todo el historial</option></select>
    ...
    <script>
        ...
        async function loadDashboard() {
            ...
            if (from) urlMetrics += \`?from=\${new Date(from).getTime()}\`;
            if (to) urlMetrics += \`\${from?'&':'?'}to=\${new Date(to).getTime()}\`;
            if (from) urlMessages += \`?from=\${new Date(from).getTime()}\`;
            if (to) urlMessages += \`\${from?'&':'?'}to=\${new Date(to).getTime()}\`;
            try {
        ...
        function renderAB(ab) {
            ...
            abChart = new Chart(ctx, {
                type: 'bar',
                data: { labels: ['Net Revenue / sesión (USD)'], datasets: [
        ...
        function downloadCSV() {
            ...
            for (const m of currentData.messages) csvRows.push([m.messageId, m.impressions, m.netRevenue, m.netPerImpression]);
