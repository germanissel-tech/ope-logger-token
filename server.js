...
let configCache = new Map(); // key -> { messages, weights, expiresAt }
...
app.get("/config/:storeId", (req, res) => {
  ...
  // Pesos de política (por defecto, pero el frontend puede usar los suyos)
  const weights = {
    ASSIST: 0.5,
    SOCIAL: 2.5,
    URGENCY: 0.05,
...
app.post("/config/:storeId", auth, (req, res) => {
  ...
  if (weights) {
    // Guardar pesos en alguna tabla de configuración (opcional)
...
app.get("/config/weights/:storeId", (req, res) => {
  const storeId = safe(req.params.storeId);
  if (!storeId) return res.status(400).json({ error: "storeId required" });
  // Por ahora devolvemos pesos por defecto; se pueden sobreescribir vía tabla
  const weights = {
    ASSIST: 0.5,
    SOCIAL: 2.5,
    URGENCY: 0.05,
